...
namespace LiquidGlassAvaloniaUI
{
    internal static class LiquidGlassBackdropProvider
    {
        ...
        private sealed class BackdropState
        {
            ...
            public ConditionalWeakTable<Control, CapturedLayout> CapturedLayouts { get; } = new();
        ...
        public static void EnsureSnapshot(Control control)
        {
            ...
            bool layoutChanged = TryCalculateControlVisualBounds(control, topLevel, out Rect bounds)
                && (!state.CapturedLayouts.TryGetValue(control, out CapturedLayout? layout)
            ...
            if (shouldCapture)
            {
                // A resized lens must never sample the previous layout for one frame.
                // Ordinary background updates still use the throttled frame capture below.
                if (Dispatcher.UIThread.CheckAccess())
                    Capture(topLevel, state, deferInvalidation: true);
                else
        ...
        private static void Capture(TopLevel topLevel, BackdropState state, bool deferInvalidation = false)
        {
            if (IsCapturing)
            {
            ...
            // EnsureSnapshot may run inside Render; Avalonia forbids invalidating visuals there.
            if (deferInvalidation)
                Dispatcher.UIThread.Post(() => InvalidateSubscribers(state), DispatcherPriority.Render);
            else
                InvalidateSubscribers(state);
        }

        private static void RecordCapturedLayouts(TopLevel topLevel, BackdropState state)
        {
