...
public static class VirtualPath
{
    ...
    public static (string ServerId, string RemotePath) ParseRemotePath(string path)
    {
        var withoutPrefix = path[RemotePrefix.Length..];
        var colonIndex = withoutPrefix.IndexOf(':');
        if (colonIndex < 0) return (withoutPrefix, "/");
        return (withoutPrefix[..colonIndex], withoutPrefix[(colonIndex + 1)..]);
    }

    public static string BuildRemotePath(string serverId, string remotePath)
    {
        var normalized = remotePath.Replace('\\', '/');
        if (!normalized.StartsWith('/')) normalized = "/" + normalized;
        return $"{RemotePrefix}{serverId}:{normalized}";
    }
}