...
public sealed record FileOperationResult(IReadOnlyList<string> CompletedPaths,
    IReadOnlyList<string> FailedPaths, IReadOnlyList<string> Warnings,
    IReadOnlyList<string>? NotAttemptedPaths = null);

public sealed class FileOperationPartialException(FileOperationResult result, Exception inner)
    : IOException($"操作未完整完成：已完成 {result.CompletedPaths.Count} 项，失败 {result.FailedPaths.Count} 项。{inner.Message}", inner)
{
    public FileOperationResult Result { get; } = result;
}
    ...
    ILogger? logger)
{
        ...
        IReadOnlyList<string> sourcePaths, string destinationDirectory)
    {
        ...
        try
        {
            ...
            for (var index = 0; index < sourcePaths.Count; index++)
            {
                ...
                {
                    try { await tags.CopyPathAsync(sourcePaths[index], destination, CancellationToken.None); }
                    catch (Exception ex)
                    {
                        warnings.Add($"{destination}：标签未同步（{ex.Message}）");
                        logger?.LogError(ex, "Failed to copy tags for {Path}", destination);
                    }
                }
            }
            token.ThrowIfCancellationRequested();
            if (task != null)
            {
                try { tasks!.CompleteTask(task.Id); }
                catch (Exception ex)
                {
                    warnings.Add($"任务进度状态未更新：{ex.Message}");
                    logger?.LogError(ex, "Failed to complete copy task status");
                }
            }
            return new(outputs, failed, warnings);
        }
        catch (OperationCanceledException) when (token.IsCancellationRequested)
        {
            if (task != null) tasks!.CancelTask(task.Id);
            throw new OperationCanceledException($"复制已取消；已完成：{string.Join("、", outputs)}", token);
        }
        catch (Exception ex)
        {
            if (task != null)
            ...
            if (outputs.Count < sourcePaths.Count) failed.Add(sourcePaths[outputs.Count]);
            throw new FileOperationPartialException(new(outputs, failed, warnings,
                sourcePaths.Skip(outputs.Count + 1).ToArray()), ex);
        }
        ...
        Action<string>? setStatus = null)
    {
        ...
        if (warnings.Count > 0) setStatus?.Invoke($"已移动到 {movedPath}；{string.Join("；", warnings)}");
        return new([movedPath], [], warnings);
    }
}
