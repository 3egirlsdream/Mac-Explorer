    ...
    HttpMessageHandler inner, CopilotStore store, Func<string> sessionId) : DelegatingHandler(inner)
{
    ...
    private async Task RestoreRequestAsync(HttpRequestMessage request, string requestSessionId,
        CancellationToken cancellationToken)
    {
        ...
        var saved = store.LoadReasoning(requestSessionId).Select(item => new ReplayEntry(
        ...
        Stream inner, HttpContent original, CopilotStore store, string sessionId) : Stream
    {
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            Capture(buffer.AsSpan(offset, read), read == 0 && count > 0);
            return read;
        }
        ...
        public override async ValueTask<int> ReadAsync(Memory<byte> buffer, CancellationToken cancellationToken = default)
        {
            ...
            Capture(buffer.Span[..read], read == 0 && !buffer.IsEmpty);
            return read;
        }
