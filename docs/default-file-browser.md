...
FKFinder 提供"设为默认文件浏览器"功能，允许用户将 FKFinder 注册为 macOS 系统中 `public.folder` 类型的默认处理程序。该功能通过 macOS LaunchServices API（`LSSetDefaultRoleHandlerForContentType`）实现注册，并在 `Info.plist` 中声明 `CFBundleDocumentTypes` 以支持文件夹类型。

核心挑战在于：当系统通过 URL 启动或唤醒 FKFinder 时，需要在三种不同的进程状态下正确接收并导航到目标文件夹路径。
...
将 FKFinder 设为默认文件夹处理程序后，以下场景会使用 FKFinder 打开文件夹：
