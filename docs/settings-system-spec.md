...
外观设置分为两类：**即时生效**（深色模式）和**重启生效**（毛玻璃效果）。
...
当前仅开放"跟随系统"选项，自动跟随 macOS 系统外观。`IThemeService` 接口保留了 `SetThemeMode("light"/"dark")` 能力，未来可在 UI 中开放手动选择。

深色模式通过 CSS 变量实现，切换后立即生效，无需重启。
...
透明度滑块在毛玻璃关闭时为 disabled 状态。底部显示"毛玻璃设置修改后需重启应用生效"提示。

深色模式通过 `IThemeService` 实时生效，毛玻璃效果由 `App.xaml.cs` 在启动时读取并传递给 `VibrancyHelper` 静态属性。详见 `docs/mac-catalyst-transparent-window.md`。

**下拉框关闭机制**：主题下拉框使用 backdrop 遮罩模式——仅在下拉框打开时渲染一个 `position:fixed; inset:0` 的透明遮罩层（`.settings-dropdown-backdrop`），点击遮罩关闭下拉框。不使用父容器 `@onclick`，以避免触发不必要的 Blazor 重渲染影响滑块等控件的交互。

## 涉及文件

| 文件 | 职责 |
|------|------|
| `Components/Dialogs/SettingsDialog.razor` | 设置 UI（4 个 Tab、开关/滑块/下拉控件、事件处理） |
...
| `Components/Pages/Home.razor` | MDialog 容器（MaxWidth=480）、主题事件订阅、JS Interop |
| `wwwroot/css/app.css` | Tab 样式、分组标题、紧凑项、滑块样式、下拉控件样式 |
| `wwwroot/css/variables.css` | CSS 变量定义（浅色/深色模式） |
| `wwwroot/js/theme.js` | Web 端主题切换逻辑、系统主题监听 |
| `App.xaml.cs` | 注入 ISettingsService，启动时读取毛玻璃配置和初始化 ThemeService |
...
/* 外观 Tab — 下拉选择（工具栏样式统一） */
.settings-dropdown-wrapper   /* 相对定位容器，@onclick:stopPropagation 防止冒泡 */
.settings-dropdown-backdrop  /* 透明全屏遮罩，仅下拉打开时渲染，点击关闭下拉 */
.settings-dropdown-trigger   /* 触发按钮，工具栏下拉样式 */
.settings-dropdown           /* 下拉菜单，glass-effect */
.settings-dropdown-item      /* 选项项 */
.settings-dropdown-item.active /* 选中态 */
.settings-dropdown-check     /* 选中标记 ✓ */

/* 重启提示 */
.settings-restart-hint  /* 11px，居中，tertiary 文字色，仅用于毛玻璃设置 */
```
...
1. **创建服务接口** — 如 `IThemeService`，定义事件和属性：
   ```csharp
...
2. **实现平台服务** — 如 `MacThemeService`，处理平台特定的检测逻辑：
   - 使用 `UITraitCollection` 检测系统主题
   - 监听 `NSSystemAppearanceChangedNotification` 系统通知
   - 触发 `ThemeChanged` 事件通知 UI 更新

3. **Web 端支持** — 创建 `theme.js` 处理 CSS 类切换：
   ```javascript
   window.applyTheme = function(mode) { /* ... */ };
   window.setDarkMode = function(isDark) { /* ... */ };
   ```

4. **Blazor 组件订阅** — 在 `Home.razor` 中：
   ```csharp
...
5. **CSS 变量方案** — 使用 CSS 变量定义浅色/深色颜色，通过 `body.dark-mode` 切换：
   ```css
   :root { --color-bg: #ffffff; }
   .dark-mode { --color-bg: #1e1e1e; }
   ```

6. **设置 UI** — 在 `SettingsDialog.razor` 中添加下拉选择，调用 `ThemeService.SetThemeMode()` 即时生效。
