...
export async function readManifest(file) {
  ...
  if (tail.getUint16(end + 4, true) !== 0 || tail.getUint16(end + 6, true) !== 0 ||
      tail.getUint16(end + 8, true) !== entries || entries > 10000) throw new Error('不支持分卷或超大 ZIP 插件包。');
  if (length > 16 * 1024 * 1024 || offset + length > tailStart + end) throw new Error('插件目录过大或无效。');
  const bytes = new Uint8Array(await file.slice(offset, offset + length).arrayBuffer());
  const view = new DataView(bytes.buffer); let manifest = null, seen = 0;
  ...
  if (seen !== entries) throw new Error('插件目录条目数不匹配。');
  if (!manifest || manifest.unpacked > 1024 * 1024 || manifest.size > 1024 * 1024) throw new Error('缺少清单或清单过大。');
  if (manifest.local + 30 > offset) throw new Error('清单数据越界。');
  ...
  if (start + manifest.size > offset || local.getUint16(8, true) !== manifest.method ||
      (local.getUint16(6, true) & 1)) throw new Error('清单数据越界或格式不一致。');
  const localName = await file.slice(manifest.local + 30, manifest.local + 30 + local.getUint16(26, true)).text();
  if (localName !== 'plugin.json') throw new Error('清单名称不一致。');
  ...
  if (total !== manifest.unpacked) throw new Error('清单解压长度不匹配。');
