...
function renderAccount() {
  ...
  $('verified-label').textContent = session?.verified ? '邮箱已验证 · 可以发布插件' : '验证邮箱后可发布插件';
...
$('account-form').onsubmit = async event => {
  ...
  try {
    const data = await api(mode === 'register' ? 'Register' : mode === 'recover' ? 'Recover' : 'Login', { email, password, displayName: $('display-name').value, code: $('code').value.trim() });
    if (mode === 'recover') { setMode('login'); message('account-message', '密码已重置，请重新登录。'); }
    else { message('account-message','登录成功。'); saveSession({ ...data, email }); $('password').value = ''; $('confirm-password').value = ''; await loadMine(); }
  } catch (error) { message('account-message', error.message, true); }
  finally { busy = false; $('submit-account').disabled = false; renderAccount(); }
};
async function sendCode() {
  if (countdown) return;
  const email = session?.email || $('email').value.trim();
...
$('verify-email').onclick = async () => { try { await api('VerifyEmail', { code: $('verify-code').value.trim() }); saveSession({ ...session, verified: true }); message('account-message', '邮箱已验证。'); } catch (error) { message('account-message', error.message, true); } };
$('logout').onclick = async () => { try { await api('Logout', {}); } finally { saveSession(null); $('my-plugins').textContent = '登录后查看你发布的插件。'; } };
$('package').onchange = async () => {
  selected = null; $('preview').hidden = true; renderAccount(); message('upload-message', '正在读取插件清单…');
  const file = $('package').files[0]; if (!file) return;
  try {
    const manifest = await readManifest(file); if ($('package').files[0] !== file) return;
    selected = { file, manifest }; $('preview').hidden = false; $('plugin-name').textContent = manifest.name; $('plugin-description').textContent = manifest.description || '暂无描述'; $('plugin-meta').replaceChildren();
    ...
    message('upload-message','清单已读取，发布前将由服务器再次校验。');
...
$('publish').onclick = async () => {
  ...
  const { file, manifest } = selected; busy = true; renderAccount(); $('package').disabled = true; $('upload-progress').hidden = false;
  try {
  ...
  finally { busy = false; $('package').disabled = false; $('upload-progress').hidden = true; renderAccount(); }
};
async function loadMine() {
  if (!session) return;
  try {
    const items = await api('Mine'); $('my-plugins').replaceChildren();
    if (!items.length) $('my-plugins').textContent = '还没有发布插件。选择一个插件包开始。';
