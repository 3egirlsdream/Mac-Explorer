...
(function () {
    ...
    document.addEventListener('dragstart', function (event) {
        ...
        if (urls.length > 0) {
            try {
                ...
                console.log('[FKFinder/Drag] Set ' + urls.length + ' file URL(s)');
            } catch (e) {
                console.warn('[FKFinder/Drag] setData failed:', e);
            }
        }
    ...
    document.addEventListener('drop', function (event) {
        ...
        console.log('[FKFinder/Drop] External drop. types:', Array.from(event.dataTransfer.types));
        ...
        if (paths.length === 0) {
            try {
                ...
                if (d) { console.log('[FKFinder/Drop] text/uri-list:', d.substring(0, 300)); }
        ...
        if (paths.length === 0) {
            try {
                ...
                if (d) { console.log('[FKFinder/Drop] text/plain:', d.substring(0, 300)); }
                paths = extractFilePathsFromString(d);
            } catch (e) { }
        }
        ...
        if (paths.length === 0) {
            try {
                ...
                if (d) { console.log('[FKFinder/Drop] URL:', d); }
                paths = extractFilePathsFromString(d);
            } catch (e) { }
        }
        ...
        if (paths.length === 0) {
            try {
                ...
                if (d) { console.log('[FKFinder/Drop] public.file-url:', d); }
                paths = extractFilePathsFromString(d);
            } catch (e) { }
        }
            ...
            for (var i = 0; i < event.dataTransfer.types.length; i++) {
                ...
                try {
                    ...
                    if (d && d.indexOf('file://') >= 0) {
                        console.log('[FKFinder/Drop] Found file:// in "' + type + '":', d.substring(0, 300));
                        paths = extractFilePathsFromString(d);
        ...
        if (paths.length === 0 && event.dataTransfer.files.length > 0) {
            for (var i = 0; i < event.dataTransfer.files.length; i++) {
                ...
                console.log('[FKFinder/Drop] File: ' + f.name + ' path=' + (p || 'N/A') + ' size=' + f.size);
        ...
        if (paths.length > 0) {
            if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.fkfinderDrop) {
                ...
                console.log('[FKFinder/Drop] Sent ' + paths.length + ' path(s) via messageHandler');
            } else {
                console.error('[FKFinder/Drop] fkfinderDrop handler not available!');
            }
        } else {
            console.warn('[FKFinder/Drop] No file paths extracted. Dump all data:');
            for (var i = 0; i < event.dataTransfer.types.length; i++) {
                var type = event.dataTransfer.types[i];
                if (type === 'Files') { console.log('[FKFinder/Drop]   Files count:', event.dataTransfer.files.length); continue; }
                try {
                    var d = event.dataTransfer.getData(type);
                    console.log('[FKFinder/Drop]   "' + type + '":', d ? d.substring(0, 500) : '(empty)');
                } catch (e) { console.log('[FKFinder/Drop]   "' + type + '": error'); }
            }
        }
    }, true);
